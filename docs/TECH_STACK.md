...
- `src-tauri/tauri.conf.json` 当前启用 `downloadBootstrapper`；安装包优先复用系统 WebView2，缺失时再下载并拉起 bootstrapper 安装。`build.ps1` / `scripts/build-helper.ps1` 不再要求 `src-tauri/webview2-runtime/`，`src-tauri/build.rs` 也不再为 fixed runtime 做预处理。便携包统一依赖系统 WebView2
