...
fn main() {
    ...
    let key_pair =
        Ed25519KeyPair::from_pkcs8(pkcs8_bytes.as_ref()).expect("Failed to parse generated key");
    let public_key = key_pair.public_key().as_ref();

    // Write private key (PKCS#8 format)
    fs::write("keys/modem-cat.sk", pkcs8_bytes.as_ref()).expect("Failed to write private key");
