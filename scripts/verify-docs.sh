...
check "BUILD 记录 portable ZIP 依赖系统 WebView2" "grep -q 'portable.zip' docs/BUILD.md && grep -q '依赖系统 WebView2' docs/BUILD.md"
...
check "README 记录构建产物与适用场景" "grep -q 'portable.zip' docs/README.md && grep -q '便携包' docs/README.md && grep -q 'NSIS 安装包' docs/README.md"
