...
check "BUILD 记录 fixed WebView2 非跟仓与 app-local 路径约束" "grep -q 'src-tauri/webview2-runtime/' docs/BUILD.md && grep -q '不进入 Git' docs/BUILD.md"
check "BUILD / TECH_STACK 记录 Fixed Version CAB 而非 Evergreen 安装器" "grep -q 'Fixed Version CAB' docs/BUILD.md && grep -q 'Fixed Version CAB' docs/TECH_STACK.md"
check "BUILD / README 说明 dist/modem-cat.exe 依赖同层 webview2-runtime" "grep -q 'dist/webview2-runtime/' docs/BUILD.md && grep -q 'dist/modem-cat.exe' docs/BUILD.md && grep -q 'dist/modem-cat.exe' README.md && grep -q 'webview2-runtime' README.md"
...
check "setup-webview2 不再使用 Evergreen Standalone Installer" "! grep -q 'linkid=2124701' scripts/setup-webview2.ps1 && grep -q 'FixedVersionRuntime' scripts/setup-webview2.ps1"
