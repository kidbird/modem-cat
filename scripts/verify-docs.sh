...
check "BUILD / TECH_STACK 记录 embedBootstrapper 与系统 WebView2 约束" "grep -q 'embedBootstrapper' docs/BUILD.md && grep -q 'embedBootstrapper' docs/TECH_STACK.md && grep -q '系统 WebView2' docs/BUILD.md"
...
check "tauri.conf / build.rs / setup-webview2 已切到 embedBootstrapper 流程" "grep -q 'embedBootstrapper' src-tauri/tauri.conf.json && ! grep -q 'ensure_fixed_runtime' src-tauri/build.rs && grep -q 'embedBootstrapper' scripts/setup-webview2.ps1 && ! grep -q 'fixedRuntime' scripts/setup-webview2.ps1"
